...
__device__ double CollideTimeWall(Particle &a, double boxLength) {
    double r = a.radius;
...
__global__ void parallelRecalcSingle(Particle* pars, long id, double step, long n, long l, long K, double cur)
{
    long i = threadIdx.x * K;
    for (long ct = 0; i + ct < n && ct < K; ct++) {
        if (i + ct == id) {
            ...
            long pos = atomicAdd((unsigned long long*) &cnt, (unsigned long long) 1)+1;
            colls[pos] = Collision( id, -1, wallTime, sum );
        } else {
            ...
            long pos = atomicAdd((unsigned long long*) &cnt, (unsigned long long) 1)+1;
            colls[pos] = Collision( id, i+ct, colTime, sum );
        }
    }
}

__global__ void parallelRecalcDouble(Particle* pars, long id1, long id2, double step, long n, long l, long K, double cur)
{
    long i = threadIdx.x * K;
    for (long ct = 0; i + ct < n && ct < K; ct++) {
        long p = i + ct;
        if (p == id1) {
            ...
            long pos = atomicAdd((unsigned long long*) &cnt, (unsigned long long) 1)+1;
            colls[pos] = Collision( id1, -1, wallTime, sum );
        } else if (p == id2) {
            ...
            long pos = atomicAdd((unsigned long long*) &cnt, (unsigned long long) 1)+1;
            colls[pos] = Collision( id2, -1, wallTime, sum );
        } else {    
            ...
            if (colTime1 + EPS >= cur && colTime1 <= step + EPS) {
                ...
                long pos = atomicAdd((unsigned long long*) &cnt, (unsigned long long) 1)+1;
                colls[pos] = Collision( id1, p, colTime1, sum );
            }
            ...
            if (colTime2 + EPS >= cur && colTime2 <= step + EPS) {
                long sum = pars[id2].wColl + pars[id2].pColl
                    + pars[p].wColl + pars[p].pColl;
                long pos = atomicAdd((unsigned long long*) &cnt, (unsigned long long) 1)+1;
                colls[pos] = Collision( id2, p, colTime2, sum );
            }
...
__host__ void perf_resolve(Particle* host_pars, Particle* device_pars_ptr, double step) 
{
    ...
    while (!heap.isEmpty()) {
        ...
        if (col.par2 == -1) {
            ...
            parallelRecalcSingle<<<1, split>>>(device_pars_ptr, id, step, host_n, host_l, k, curTime);
            ...
            parallelRecalcDouble<<<1, split>>>(device_pars_ptr, id1, id2, step, host_n, host_l, k, curTime);
            cudaDeviceSynchronize();
            cudaMemcpyFromSymbol(&heap.n, cnt, sizeof(long));
            heap.build();
...
int main(int argc, char** argv)
{
    ...
    long K = host_n <= 1e5 ? 1 << max(5, (int)floor(log2(host_n)) / 2) : 1 << 10;
